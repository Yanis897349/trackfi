  ...
  return (
    <Card className="gap-3 rounded-[10px] border-0 bg-neutral-900 py-[18px] text-white shadow-none">
      <CardHeader className="flex flex-row items-start justify-between px-[18px]">
        <div>
          <CardTitle className="text-sm">Budget pace</CardTitle>
          <p className="mt-1 text-[11px] text-neutral-400">
            {formatShortDateOnly(summary.period.start)}–
            {formatShortDateOnly(summary.period.end)} · day{" "}
            {summary.period.elapsedDays} of {summary.period.totalDays}
          </p>
        </div>
        {summary.pace && (
          <span className="flex items-center gap-1.5 rounded-full bg-white/10 px-2 py-1 text-[10px] font-medium text-amber-300">
            ...
            {displayLabel(summary.pace)} pace
          </span>
        )}
      </CardHeader>
      <CardContent className="space-y-3 px-[18px]">
        {budget === null ? (
          <div className="rounded-lg bg-white/5 p-4 text-sm text-neutral-300">
            Set a monthly budget to compare actual spending with your target.
          </div>
        ) : (
          <>
            <div className="flex items-end justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] text-neutral-400">target by today</p>
              </div>
            </div>
            <div
            ...
            <p className="flex items-center gap-2 text-[11px] text-neutral-300">
              <SparklesIcon className="size-3.5 text-amber-300" /> Hold daily
              spend below{" "}
              {formatMoney(summary.recommendedDailyMinor ?? 0, currency)} to
              finish on budget.
            </p>
          </>
        )}
